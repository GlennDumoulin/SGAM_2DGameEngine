...
GameManager::GameManager()
{
	...
	std::function<void()> goToMenuFunc{
		[]() {
			sgam::SceneManager::GetInstance().SetNextScene("MenuScene");
		}
	};
	m_pGoToMenuCommands.push_back(inputManager.BindKeyboardCommand(
		SDL_SCANCODE_ESCAPE, sgam::InputManager::InputType::ButtonDown,
		std::make_unique<sgam::FunctionCommand>(goToMenuFunc)
	));
	m_pGoToMenuCommands.push_back(inputManager.BindControllerCommand(
		0, sgam::InputManager::ControllerButton::START, sgam::InputManager::InputType::ButtonDown,
		std::make_unique<sgam::FunctionCommand>(goToMenuFunc)
	));
}
...
GameManager::~GameManager()
{
	...
	// Unbind Commands
	inputManager.UnbindCommand(m_pSkipLevelCommand);
	for (auto pCommand : m_pGoToMenuCommands)
	{
		inputManager.UnbindCommand(pCommand);
	}
}

void GameManager::Reset(int nrOfPlayers)
{
	m_pPlayers.clear();
